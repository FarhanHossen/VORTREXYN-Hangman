...
function getDeploymentDomain() {
  ...
  console.error(
    "ERROR: No deployment domain found. Set INTERNAL_APP_DOMAIN, DEV_DOMAIN, or EXPO_PUBLIC_DOMAIN",
  );
  process.exit(1);
}
...
function getAppBuildId() {
  // Used to uniquely tag this build environment; falls back to EXPO_PUBLIC_BUILD_ID
  return process.env.APP_BUILD_ID || process.env.REPL_ID || process.env.EXPO_PUBLIC_REPL_ID || process.env.EXPO_PUBLIC_BUILD_ID;
}

async function startMetro(expoPublicDomain, appBuildId) {
  ...
  const env = {
    ...
    EXPO_PUBLIC_BUILD_ID: appBuildId,
  };

  if (appBuildId) {
    console.log(`Setting EXPO_PUBLIC_BUILD_ID=${appBuildId}`);
  }
...
async function main() {
  ...
  await startMetro(domain, appBuildId);
